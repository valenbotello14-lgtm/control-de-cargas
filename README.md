...
- **Progresión por ejercicio (sobrecarga progresiva)**: en el perfil de cada alumno (coach), cada ejercicio tiene su propia tabla con **todo el historial de peso × repeticiones (y RIR) sesión a sesión**, con la variación (▲/▼ kg) contra el registro anterior. En "Mi Progreso" (Modo Jugador) se ve como una lista de tarjetas más fácil de leer — un ícono ▲/▼/● y el kg de diferencia contra la sesión anterior de ese mismo ejercicio, en vez de una tabla.
- **Evolución de Rendimiento**: gráfico por ejercicio que agrupa los registros de cada alumno por **semana, mes o año** (elegible con un chip), con dos métricas para elegir — **RM estimado** o **Volumen (kg totales levantados)**. Arriba del gráfico se ve el número actual grande con una etiqueta de tendencia ("▲ +8% este mes" / "▼ 5% este mes", en verde o ámbar — nunca rojo), y el punto más reciente del gráfico queda marcado con su valor en kg al lado, sin necesidad de tocarlo. Con la métrica RM estimado, además aparece un cartel de **marca personal** ("🏅 ¡Estás en tu mejor marca de siempre!" o "Te faltan X kg para tu mejor marca") — que **no se muestra si esa marca viene bajando**, para que nunca lea como un reproche. Abajo sigue el resumen en texto con las fechas exactas ("▲ Mejoró 15%" / "▼ Bajó 8%", primer vs. último período). Disponible tanto en el perfil del atleta (coach) como en "Mi Progreso" (Modo Jugador).
